...
/**
 * An empty workspace and no site identity.
 *
 * injectGrokPwaHead falls back to reading src/lib/og/site.json and public/og.jpg
 * from `cwd` when the caller passes neither, which is right in production and
 * wrong in a unit test: once this workspace was branded, every test that omitted
 * them started asserting against Theos Logos's own title and card instead of the
 * precedence rule it meant to check. Tests that care about the fallbacks pass a
 * populated cwd of their own; these pass an empty one.
 */
function bare() {
  return { site: {}, cwd: mkdtempSync(join(tmpdir(), "grok-og-bare-")) };
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", ...bare() });
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", ...bare() });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", ...bare() });
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", ...bare() });
...
test("site.json title brands the install name and icons", () => {
  ...
  assert.equal(manifest.theme_color, "#fffdf8");
