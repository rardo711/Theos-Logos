...
/** Drop parenthetical elaborations like "(image)" so they do not steal chip keys. */
function primaryGlosses(raw) {
  const out = [];
  for (const g of raw || []) {
    const t = cleanText(g, 80);
    if (!t) continue;
    if (/^\(.*\)$/.test(t)) continue;
    out.push(t);
  }
...
async function loadJson(localName, url) {
  ...
  try {
    ...
    const res = await fetch(url, {
      headers: {
        "User-Agent":
          "Theos-Logos EN lexicon importer (CC BY-SA attribution)",
      },
    });
...
function parseUbs(entries) {
  ...
  for (const e of entries) {
    const codes = [
      ...new Set((e.StrongCodes || []).map(normalizeStrongs).filter(Boolean)),
    ];
    ...
    for (const bf of e.BaseForms || []) {
      ...
      for (const lm of bf.LEXMeanings || []) {
        ...
        const code =
          lm.LEXEntryCode != null ? String(lm.LEXEntryCode).slice(0, 16) : "";
        const rv = [
          ...new Set(
            (lm.LEXReferences || []).map(silVerseKey).filter(Boolean),
          ),
        ];
        for (const sense of lm.LEXSenses || []) {
          if ((sense.LanguageCode || "en") !== "en") continue;
          const glosses = primaryGlosses(sense.Glosses || []);
...
async function main() {
  ...
  process.stderr.write(
    `  eng.tsv kept ${kept}, llm-only skipped ${skipped}, unique ${Object.keys(short).length}\n`,
  );
  ...
  process.stderr.write(
    `  eng attached to UBS ${engFilled}, eng-only gaps ${engOnly}\n`,
  );
  ...
  process.stderr.write(
    `  sample G3056 glosses: ${JSON.stringify(sample?.ss?.[0]?.g)}\n`,
  );
  const img = by.G1504;
  process.stderr.write(
    `  sample G1504 senses: ${JSON.stringify(img?.ss?.map((s) => ({ g: s.g, code: s.code, hasCol: (s.rv || []).includes("051001015") })))}\n`,
  );
}
