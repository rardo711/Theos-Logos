...
/**
 * `.grok/` is gitignored except for app-env.json, so the og skill's own docs
 * are never present in a clone or in CI -- only in a workspace where the Grok
 * tooling installed them. These tests pin prose in those docs to constants in
 * this repo, which is worth doing where the docs exist and is not a failure of
 * this repository where they do not. Skipping keeps the suite honest: a check
 * that can never pass in CI teaches people to ignore a red run.
 */
const OG_SKILL_DIR = join(TEMPLATE_ROOT, ".grok/skills/og");
const noOgSkill = existsSync(OG_SKILL_DIR)
  ? false
  : "the og skill is not installed in this workspace (.grok/ is gitignored)";

test("SKILL.md and AGENTS.md name the marker path and bound this script uses", { skip: noOgSkill }, () => {
...
test("the sections that own the brand-task prohibition never affirm a wait", { skip: noOgSkill }, () => {
...
test("SKILL.md tells the pass to self-check with the flag this CLI accepts", { skip: noOgSkill }, () => {
